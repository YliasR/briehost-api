...
#define USER_ADDR 64      // ユーザ名の開始アドレス
#define PASSWORD_ADDR 80  // パスワードの開始アドレス

// プロトタイプ宣言を追加
int base64Decode(String encoded, uint8_t *decoded);
...
void startCameraServer() {
  ...
  // Stream server with a different port
  config.server_port = 81;  // Set this to the desired port number
  config.ctrl_port += 1;
  log_i("Starting stream server on port: '%d'", config.server_port);
  if (httpd_start(&stream_httpd, &config) == ESP_OK) {
    httpd_register_uri_handler(stream_httpd, &stream_uri);
