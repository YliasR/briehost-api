...
1. **ClamAV scan (required baseline)**
   ...
   - Prefer daemon mode (`clamd`) for performance.
    - If you use `CLAMD_HOST=127.0.0.1` and `CLAMD_PORT=3310`, make sure `clamd` is configured with `TCPSocket 3310` and `TCPAddr 127.0.0.1`; otherwise it may listen only on a Unix socket.
